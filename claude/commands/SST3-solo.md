...
- **Pre-Ralph mechanical vacuity sweep (#567 AC 4.3, parity with Leader.md step 6.5(0))**: run `bash $SST3/sst3-test-vacuity.sh` over the diff's test files (plus `--count-in <doc>` for any allowlist-shaped dict the diff touches) and FIX every finding BEFORE dispatching any Ralph tier — exit 1 blocks Tier-1 dispatch (vacuous assertions are script-detectable; spending a reviewer on them is the measured waste class).
