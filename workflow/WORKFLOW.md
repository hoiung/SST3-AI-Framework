...
- [ ] **Workflow tool / ultracode (optional authoring aid, #507)**: this swarm MAY be authored/run via the Workflow tool in `/effort ultracode`; AP #14 still governs — the main agent verifies every finding against source, and the Layer-2 adversarial angle stays raw-tools-only. The Workflow tool's worktree-isolation default is UNVERIFIED — test it before relying on it. Not a stage rewrite; the swarm contract is unchanged.
...
- [ ] **Workflow tool / ultracode (optional authoring aid, #507)**: this post-implementation swarm MAY be authored/run via the Workflow tool in `/effort ultracode`; AP #14 still governs (main agent verifies every finding; the Layer-2 raw-tools-only angle stays raw-tools-only); worktree-isolation default is UNVERIFIED — test before relying on it. Not a stage rewrite.
