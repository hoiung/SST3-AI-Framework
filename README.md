...
- **16 Pre-Commit Enforcement Hooks**: Token budget monitoring, mirror drift detection, hardcoded parameter detection, public-repo secret scan, plus standard hygiene checks (trailing whitespace, large-file guard, JSON/YAML/TOML validity)
...
Every rule, standard, template, script, and anti-pattern lives in ONE canonical place inside `dotfiles/SST3/`. Other repos (this one, `ebay-seller-tool`, `hoiboy-uk`) vendor copies — byte-identical for Python scripts, scrubbed (private paths, identifiers, trading-specific terms genericized) for markdown documentation. A manifest-driven pre-commit hook (`check-mirror-drift.py`) fails any mirror commit where a vendored copy has drifted from canonical after the expected transforms (or, for hand-authored scrubs, where the mirror sha256 no longer matches the recorded hash). A companion pre-commit hook in the canonical repo (`propagate-mirrors.py --validate`) covers the other direction: for transform-mode files it fails any canonical edit that has not been propagated. Divergent-mode (hand-scrubbed) mirrors are hash-pinned only; their mirrors must be hand-edited in the same commit and the hash refreshed with `propagate-mirrors.py --apply`. No "which version is the real one?" ambiguity. That IS the "Single Source of Truth v3" in the name, applied at the file-system level.
...
| Metric | Value |
|--------|-------|
| Commits produced | 10,000+ across the dotfiles canonical + 12 consumer mirrors |
...
├── MIRROR-CONTRACT.md     # Canonical-mirror governance contract for adopters
├── .pre-commit-config.yaml # 16-hook enforcement config
...
- **`gate-2-merge.md`** — the recursion-safe remote fast-forward merge procedure
- **`gate-3-user-review.md`** — the operator-facing review checklist gate
- **`three-tier-testing.md`** — the Unit / Workflow / E2E tier USE rules
- **`contract-verification.md`** — cross-boundary contract checks (type / schema / config)
...
| `MIRROR-CONTRACT.md` | Mirror governance doc | Canonical-mirror propagation contract for adopters |
