...
> - [ ] **Three-Tier test gate** (canonical: STANDARDS.md "Three-Tier Testing Framework"; WORKFLOW.md "Verification Loop"). BUILD = always all 3; USE = all 3 RUN (fire & pass) every change (the only non-run is a documented `structural-inapplicable: <reason>`). Confirm Stage 4 will satisfy each + record `tiers: U=.. W=.. E2E=.. | BUILD-evidence:<file:line per tier>`:
>   - [ ] **Unit Tier** — every changed/added unit (callable / response field / config key) gets a checked-in test (cog/piston QC); RUNS every change.
>   - [ ] **Workflow Tier (AP #18 sample-invocation gate)** — a workflow/integration test + a real-CLI sample invocation (real DB, 8-item liquid basket; or wrapper-script ≥3-repo-shape + raw-tool counter-query) EXIST; RUNS every change — the AP #18 real-CLI sample is the Workflow Tier when the Issue touches pipeline / data-processing / orchestration / CLI-wiring / cross-module function-arg propagation / persistent-state write / `sst3-*.sh` wrapper change. Unit + smoke alone INSUFFICIENT; mocks MUST assert explicit `call_args.kwargs[...]` — never `**kwargs`-swallowing. `structural-inapplicable: <reason>` only when there is genuinely no cross-unit surface.
>   - [ ] **E2E Tier** — an end-to-end/system test EXISTS; RUNS every change, sized to scope — full real-system (real DB + real downstream + live invocation) when the Issue affects how whole components connect end-to-end (multi-component / cross-repo contract / orchestration / persistent-state spanning the pipeline / a real downstream consumer's contract); else a small backtest or an actual execution change + cleanup. The three compose, none substitutes. `structural-inapplicable: <reason>` only when there is genuinely no end-to-end surface.
